...
(defn init-channel [game]
  (assoc game ::channel (ConcurrentLinkedQueue.)))
...
(defn loading-zone [game]
  (let [loading-ch ^ConcurrentLinkedQueue (::channel game)
            ...
            (doseq [{:keys [esse-id load-fn]} to-loads]
              (try
                (let [loaded-facts (load-fn)]
                  (.add loading-ch {:esse-id esse-id :new-facts loaded-facts}))
                (catch Throwable err
                  (println esse-id "load error! cause:" (:cause (Throwable->map err)))
                  (.add loading-ch {:esse-id esse-id :new-facts [[esse-id ::state :error]]}))))))
        (update game ::world/this
                (fn [world]
